...
func (lf *LiveFlow) startSessionLoops(
	...
	go func() {
		defer wg.Done()
		lf.senderLoop(cancelCtx, invCtx, conn, queue, ts, eventCh, turnResetCh)
		_ = conn.Close()
	}()
...
// freshly opened live connection. Only consulted on the initial connect
// (handle == ""); resumed connections trust the server-side replay and skip
// history altogether — the gate at runSession composes with this routing.
...
// override wins when set; otherwise gemini-3.x defaults to batched (avoids
// 1008 policy violation on per-turn replay) and all other models keep the
// legacy per-turn replay.
...
func (lf *LiveFlow) resolveLiveModelName(ctx agent.InvocationContext) string {
	if rc := ctx.RunConfig(); rc != nil && rc.Model != "" {
		return rc.Model
	}
	if lf.Model != nil {
...
// trackedSendBatchedHistory mirrors trackedSend's timing instrumentation
// for the batched-history path. Calls liveTimingState.recordSend so that
// reconnect heuristics observe the batched call as a real send.
func trackedSendBatchedHistory(
	ctx context.Context,
	bs model.BatchedHistorySender,
	turns []*genai.Content,
	ts *liveTimingState,
) error {
	now := time.Now()
	if err := bs.SendBatchedHistory(ctx, turns); err != nil {
		return err
	}
	ts.recordSend(now)
	return nil
...
func (lf *LiveFlow) sendHistory(
	...
	modelName := lf.resolveLiveModelName(ctx)
	...
	// Per-turn path (2.5 and earlier). Send all history turns with
	// TurnComplete=false so the model absorbs them as context without
	// responding to each one individually. Only the last turn is sent
	// with TurnComplete=true to signal that history replay is complete.
	falseVal := false
...
// realtime input on gemini-3.x batched mode to avoid the 1008 policy
// violation that occurs when client_content is sent mid-session. Anything
// else is pass-through. Multi-part turns are NOT rewritten — joining
// would require a separator choice that mutates user text.
...
func (lf *LiveFlow) sendAndSignal(
	ctx context.Context,
	conn model.LiveConnection,
...
func (lf *LiveFlow) senderLoop(
	...
	rc := invCtx.RunConfig()
	modelName := lf.resolveLiveModelName(invCtx)

	for {
		select {
		case req := <-queue.Events():
			if lf.sendAndSignal(ctx, conn, req, rc, modelName, ts, eventCh, turnResetCh) != nil {
				return
			}
		case <-queue.Done():

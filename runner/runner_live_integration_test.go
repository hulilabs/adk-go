...
// TestIntegration_GeminiLive_3x_AcceptsBatchedHistoryWithoutPolicyClose is
// the acceptance gate for the 1008 fix. It opens a real Gemini Live session
// with non-empty history on a 3.x model and verifies the server does NOT
// respond with a "1008 policy violation" close. Skipped without
// GOOGLE_API_KEY.
...
// TestIntegration_GeminiLive_25NativeAudio_BaselineWithHistory pins the
// regression baseline: the per-turn replay path must keep working against
// the real API for the 2.5 Native Audio model that this fork supports.
func TestIntegration_GeminiLive_25NativeAudio_BaselineWithHistory(t *testing.T) {
	if os.Getenv("GOOGLE_API_KEY") == "" {
		t.Skip("GOOGLE_API_KEY not set")
	}
...
func runLiveAcceptanceCheck(t *testing.T, modelName string) {
	...
	// Safety hatch: close the queue if the test runs long. Receiver-side
	// TurnComplete also closes it — whichever fires first.
	go func() {
...
// violation close from the Live API. We match on string fragments since
// the underlying genai SDK surfaces the websocket close as a wrapped
// error.
